...
router.post('/', async (req, res) => {
  try {
    const { name, phone, bloodGroup, city, address, lastDonation, is_donated_before } = req.body;
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (is_donated_before !== undefined) donor.is_donated_before = is_donated_before;
...
router.get('/', async (req, res) => {
  try {
    ...
    const donors = await Donor.find(filters).sort({ createdAt: -1 }).skip(skip).limit(parseInt(limit));
    res.json({ donors });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.delete('/:id', async (req, res) => {
  ...
  if (!req.session || !req.session.isAdmin) return res.status(401).json({ message: 'Unauthorized' });
  try {
